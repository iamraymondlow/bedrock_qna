...
import backend as demo  ### replace rag_backend with your backend filename
...
if "vector_index" not in st.session_state:
    with st.spinner(
        "📀 Wait for magic...All beautiful things in life take time :-)"
    ):  ###spinner message
        st.session_state.vector_index = (
            demo.index()
        )  ### Your Index Function name from Backend File

input_text = st.text_area("Input text", label_visibility="collapsed")
go_button = st.button("📌Learn GenAI with Raymond", type="primary")  ### Button Name
...
if go_button:
    ...
    with st.spinner(
        ...
        response_content = demo.rag_response(
            index=st.session_state.vector_index, question=input_text
        )  ### replace with RAG Function from backend file
